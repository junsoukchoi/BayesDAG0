...
Monte Carlo (MCMC) samplers targeted to Bayesian differential
zero-inflated negative binomial DAGs (DAG0s) for two-sample causal
discovery for observational zero-inflated count data. One-sample DAG0
deals with multivariate zero-inflated count data generated under a
single experimental condition. This is extended to two-sample DAG0 which
models not only zero-inflation in observational count data but also
differential networks across two experimental groups.
